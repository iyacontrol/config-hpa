...
import (
	...
	chpav1beta1 "github.com/iyacontrol/config-hpa/pkg/apis/confighpa/v1beta1"
...
const (
	...
	defaultScaleUpLimitFactor                    = 50.0
	defaultScaleDownLimitFactor                  = 20.0
)
...
func newReconciler(mgr manager.Manager) reconcile.Reconciler {
	...
	recorder := broadcaster.NewRecorder(scheme.Scheme, v1.EventSource{Component: "config-horizontal-pod-autoscaler"})
...
func updatePredicate(ev event.UpdateEvent) bool {
	oldObject := ev.ObjectOld.(*chpav1beta1.ConfigHpa)
	newObject := ev.ObjectNew.(*chpav1beta1.ConfigHpa)
...
func add(mgr manager.Manager, r reconcile.Reconciler) error {
	...
	err = c.Watch(&source.Kind{Type: &chpav1beta1.ConfigHpa{}}, &handler.EnqueueRequestForObject{}, predicate)
	if err != nil {
		return err
	}
...
func (r *ReconcileCHPA) reconcileCHPA(chpa *chpav1beta1.ConfigHpa, deploy *appsv1.Deployment) (err error) {
	defer func() {
...
func (r *ReconcileCHPA) setCurrentReplicasInStatus(chpa *chpav1beta1.ConfigHpa, currentReplicas int32) {
	r.setStatus(chpa, currentReplicas, chpa.Status.DesiredReplicas, chpa.Status.CurrentMetrics, false)
}

// setStatus recreates the status of the given HPA, updating the current and
// desired replicas, as well as the metric statuses
func (r *ReconcileCHPA) setStatus(chpa *chpav1beta1.ConfigHpa, currentReplicas, desiredReplicas int32, metricStatuses []autoscalingv2.MetricStatus, rescale bool) {
	chpa.Status = chpav1beta1.ConfigHpaStatus{
...
// minReplicas, etc...)
func (r *ReconcileCHPA) normalizeDesiredReplicas(chpa *chpav1beta1.ConfigHpa, currentReplicas int32, prenormalizedDesiredReplicas int32) int32 {
...
func convertDesiredReplicasWithRules(chpa *chpav1beta1.ConfigHpa, currentReplicas, desiredReplicas, hpaMinReplicas, hpaMaxReplicas int32) (int32, string, string) {
	...
	scaleDownLimit := calculateScaleDownLimit(chpa, currentReplicas)
	// Compute the maximum and minimum number of replicas we can have
	switch {
	case hpaMinReplicas == 0:
		minimumAllowedReplicas = 1
	case desiredReplicas < scaleDownLimit:
		minimumAllowedReplicas = int32(math.Max(float64(scaleDownLimit), float64(hpaMinReplicas)))
	...
	if desiredReplicas > scaleUpLimit {
		maximumAllowedReplicas = int32(math.Min(float64(scaleUpLimit), float64(hpaMaxReplicas)))
...
// not present.
func setCondition(chpa *chpav1beta1.ConfigHpa, conditionType autoscalingv2.HorizontalPodAutoscalerConditionType, status v1.ConditionStatus, reason, message string, args ...interface{}) {
...
func setCHPADefaults(chpa *chpav1beta1.ConfigHpa) {
	...
	if chpa.Spec.ScaleDownLimitFactor == 0.0 {
		chpa.Spec.ScaleDownLimitFactor = defaultScaleDownLimitFactor
	}
...
func calculateScaleUpLimit(chpa *chpav1beta1.ConfigHpa, currentReplicas int32) int32 {
	// returns TO how much we can upscale, not BY how much.
	return int32(float64(currentReplicas) + math.Max(1, math.Floor(chpa.Spec.ScaleUpLimitFactor/100*float64(currentReplicas))))
}

// Scaledown limit is used to maximize the downscaling rate.
func calculateScaleDownLimit(chpa *chpav1beta1.ConfigHpa, currentReplicas int32) int32 {
	return int32(float64(currentReplicas) - math.Max(1, math.Floor(chpa.Spec.ScaleDownLimitFactor/100*float64(currentReplicas))))
}
func (r *ReconcileCHPA) shouldScale(chpa *chpav1beta1.ConfigHpa, currentReplicas, desiredReplicas int32, timestamp time.Time) bool {
	if desiredReplicas == currentReplicas {
...
func (r *ReconcileCHPA) computeReplicasForMetrics(chpa *chpav1beta1.ConfigHpa, deploy *appsv1.Deployment, metricSpecs []autoscalingv2.MetricSpec) (replicas int32, metric string, statuses []autoscalingv2.MetricStatus, timestamp time.Time, err error) {
	currentReplicas := deploy.Status.Replicas
...
func (r *ReconcileCHPA) updateStatusIfNeeded(oldStatus *chpav1beta1.ConfigHpaStatus, newCHPA *chpav1beta1.ConfigHpa) error {
...
func (r *ReconcileCHPA) updateCHPA(chpa *chpav1beta1.ConfigHpa) error {
...
func getLastScaleTime(chpa *chpav1beta1.ConfigHpa) time.Time {
	lastScaleTime := chpa.Status.LastScaleTime
